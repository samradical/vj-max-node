...
var PLAYLIST_BUILDER = {
	...
	build: function() {
		...
		//or _getVinesByUserId ,  this._getVinesByTags(config.defaultVineTag)
		this._getVinesByUserId().then(function() {
	...
	_getVinesByUserId: function() {
		var defer = Q.defer();
		var pageId = 1;
		var self = this;

		function __request() {
			self._requestUserVines(pageId).then(function(results) {
				//pages of results
		...
		if (vineService.me) {
			...
			vineService.login().then(function(me) {
				__request();
			});
		}
		return defer.promise;
	...
	_requestUserVines: function(pageId, items, defer) {
		var self = this;
		items = items || [];
		var defer = defer || Q.defer();
		vineService.getMe(pageId).then(function(userResponse) {
			items.push(userResponse['data']['records']);
			if (userResponse['data']['nextPage']) {
				self._requestUserVines(userResponse['data']['nextPage'], items, defer);
			} else {
				defer.resolve(items);
			}
		});
	...
	addUserVideo: function(absolutePath, name, requirements) {
		...
		vo['userVideo'] = true;
		this.doVideoProbing(vo).then(function(voWithThumb){
				self.playlist.splice(index + self.userVideoVOs.length, 0, voWithThumb);
				self.userVideoVOs.push(voWithThumb);
		}).done();
		/*this.setDuraion(vo).then(function(voWithDur) {
			self.setThumbnail(voWithDur).then(function(voWithThumb) {
				//bumps the uservideos next on the list
			});
		}).done();*/
	},

	doVideoProbing:function(vo){
		return this.setDuraion(vo).then(function(voWithDur) {
			return self.setThumbnail(voWithDur).then(function(voWithThumb) {
				//bumps the uservideos next on the list
				return voWithThumb;
				self.playlist.splice(index + self.userVideoVOs.length, 0, voWithThumb);
				self.userVideoVOs.push(voWithThumb);
	...
	videoPrep:function(vo){
		return ffmpeg.videoPrep(vo);
	}/*,

	setDuraion: function(vo) {
		return ffmpeg.getDuration(vo['absolutePath']).then(function(duration) {
			vo['duration'] = vo['timeRemaining'] = duration;
			return vo;
		});
	},

	setThumbnail: function(vo) {
		return ffmpeg.getThumbnail(vo['absolutePath'], vo['name'] + '.jpg', config.DESTINATION).then(function(filepath) {
			console.log(filepath);
			vo['thumbnailAbsolutePath'] = filepath;
			return vo;
		});
	}*/
};
